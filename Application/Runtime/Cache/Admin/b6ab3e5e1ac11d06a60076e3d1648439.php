...
<div class="pd-20">
	...
	<table class="table table-border table-bordered table-hover table-bg">
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td><?php $arr=explode(",",$v["mid"]); foreach($arr as $item){ echo getNameById("moudle",$item).","; } ?></td>
				<td class="f-14"><a title="编辑" href="javascript:;" onclick="admin_role_edit('角色编辑','<?php echo U('admin_role_add',array('id'=>$v['id']));?>','<?php echo ($v["id"]); ?>')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="admin_del(this,<?php echo ($v["id"]); ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
function admin_del(obj,id){
    layer.confirm('确认要删除吗？',function(index){
        ...
        $.post("<?php echo U('Base/comDel');?>",{"id":id,"tableName":"limit"},function (res) {
            if (res.status=="y") {
...
function datadel(){
	...
	$.post("<?php echo U('Base/comDel');?>",{"id":strId,"tableName":"limit"},function (res) {
		if (res.status=="y"){
