...
<div class="pd-20 text-c">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel('category')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" onclick="system_category_add('添加栏目','<?php echo U('categoryAdd');?>')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加栏目</a></span> <span class="r">共有数据：<strong><?php echo (count($list)); ?></strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			...
			<tbody>
				<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><tr class="text-c" id="tr<?php echo ($v["id"]); ?>">
					...
					<td><?php echo ($k); ?></td>
					<td><input  class="input-text" type="text" value="<?php echo ($v["sort"]); ?>" onblur="saveVal(this,<?php echo ($v['id']); ?>,'sort')" style="text-align: center;"></td>
					<td class="text-l"><input  class="input-text" type="text" value="<?php echo ($v["name"]); ?>" onblur="saveVal(this,<?php echo ($v['id']); ?>,'name')"></td>
					<td><?php echo (getNameById('category',$v["pid"])); ?></td>
					<td class="f-14"><a title="编辑" href="javascript:;" onclick="system_category_edit('栏目编辑','<?php echo U('categoryAdd',array('id'=>$v['id']));?>','700','')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="system_category_del(this,<?php echo ($v['id']); ?>,'category')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
function system_category_del(obj,id,tableName){
			...
			layer.confirm('确认要删除吗？',function(index){
                $.post("<?php echo U('categoryDel');?>",{"id":id,"tableName":tableName},function (res) {
