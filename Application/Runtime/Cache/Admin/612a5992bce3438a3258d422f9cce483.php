...
<div class="pd-20 text-c">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" onclick="system_category_add('添加栏目','<?php echo U('seriesAdd');?>',950,'')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加栏目</a></span> <span class="r">共有数据：<strong><?php echo (count($list)); ?></strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			...
			<tbody>
				<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><tr class="text-c" id="tr<?php echo ($v["id"]); ?>">
					...
					<td class="text-l"><img src="/Q2/tp1611<?php echo ($v["logo"]); ?>" alt="系列logo" width="100px"></td>
					<td><?php echo (cutStr($v["description"])); ?></td>
					<td><?php echo (getNameById('category',$v["cat_id"])); ?></td>
					<td class="f-14"><a title="编辑" href="javascript:;" onclick="system_series_edit('栏目编辑','<?php echo U('seriesAdd',array('id'=>$v['id']));?>','1000','')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="system_series_del(this,<?php echo ($v['id']); ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
function system_series_del(obj,id){
			...
			layer.confirm('确认要删除吗？',function(index){
                $.post("<?php echo U('Base/comDel');?>",{"id":id,"tableName":"series"},function (res) {
...
//多个
	function datadel() {
        ...
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('Base/comDel');?>",{"id":strId,"tableName":"series"},function (res) {
                if(res.status=="y"){ 
                //删除勾选的id                   
...
function saveVal(obj,id,field_name){
    ...
    $.post("<?php echo U('Base/status');?>",{"tableName":'series',"fieldName":field_name,"primary":'id','id':id,"fieldVal":field_new},function(res){
        if (res.status=="y") {
