...
<div class="pd-20">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td>
					<?php if($v['sex'] == 1): ?>男<?php else: ?>女<?php endif; ?>
				</td>
				<td><img src="/Q2/tp1611<?php echo ($v["img"]); ?>" alt=""></td>
				<td><?php echo ($v["email"]); ?></td>
				<td class="text-l"><?php echo ($v["address"]); echo ($v["detail_address"]); ?></td>
				<td><?php if(empty($v['register'])): ?>管理员手动添加<?php else: echo (date("Y-m-d H:i:s",$v["register"])); endif; ?> </td>
				<td class="td-status">
...
function member_start(obj,id){
	layer.confirm('确认要启用吗？',function(index){
        $.post("<?php echo U('Base/status');?>",{"tableName":"user","primary":"id","id":id,"fieldVal":1,"fieldName":"active"},function (res) {
            if (res.status=="y"){
