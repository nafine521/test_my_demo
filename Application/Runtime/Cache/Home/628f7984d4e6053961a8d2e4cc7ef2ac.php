...
<div class="mt-20" style="padding-bottom: 30px;">
  <table class="table table-border table-bordered table-hover table-bg table-sort">
    ...
    <tbody>
    <?php if(is_array($add_list)): $k = 0; $__LIST__ = $add_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><tr class="text-c" id="tr<?php echo ($v["id"]); ?>">

        <td><input  class="input-text" type="text" value="<?php echo ($v["consignee"]); ?>" onblur="saveVal(this,<?php echo ($v['id']); ?>,'consignee')" style="text-align: center;"></td>
        <td><input  class="input-text" type="text" value="<?php echo ($v["tel"]); ?>" onblur="saveVal(this,<?php echo ($v['id']); ?>,'tel')" style="text-align: center;"></td>
        <td class="text-l">
          <?php echo ($v["province"]); ?>
        </td>
        <td><input  class="input-text" type="text" value="<?php echo ($v["detail_address"]); ?>" onblur="saveVal(this,<?php echo ($v['id']); ?>,'detail_address')" style="text-align: center;"></td>
        <td><img src="/Q2/tp1611/public/admin/images/icon_<?php echo ($v['status']?'right':'error'); ?>_s.png" onclick="status(this,<?php echo ($v["id"]); ?>,'status',<?php echo ($v["status"]); ?>)"></td>

        <td class="f-14">
          <a title="删除" href="javascript:;" onclick="address_del(this,<?php echo ($v['id']); ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
        </td>
...
<script type="text/javascript">
    ...
    function saveVal(obj,id,field_name){
        ...
        $.post("<?php echo U('Public/status');?>",{"tableName":'address',"fieldName":field_name,"primary":'id','id':id,"fieldVal":field_new},function(res){
            if (res.status=="y") {
    ...
    function address_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            ...
            $.post("<?php echo U('Public/comDel');?>",{"id":id,"tableName":"address"},function (res) {
                if (res.status=="y") {
